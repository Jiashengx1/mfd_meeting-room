...
- PC 端可从日视图点击会议室名称进入周视图，按周一至周日查看并直接选择连续空闲时段
- 普通用户创建、查看、修改、取消自己的预约
- 普通预约和周期预约均支持代他人预约，实际部门和使用人保存为预约快照，预约权限仍归登录账号
...
- 支持每周或每两周重复，可多选星期；每两周以开始日期所在周为第一轮
- 最长范围一年
- 预约周期会议必须先预览再确认；任一表单字段变化后需要重新预览；取消周期会议需要选择周期组并二次确认
- 确认创建时后端重新检查冲突
- 冲突跳过，非冲突照常创建
- 已结束时段跳过
- 默认使用当前登录管理员的部门和姓名；启用“代他人预约”后可填写实际部门和一名主要使用人
- 代约记录仍归创建该记录的登录账号，实际使用人不会自动获得修改或取消权限
- 预约周期会议只允许选择启用会议室
